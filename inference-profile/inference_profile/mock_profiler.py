...
PREFILL_EVENT_FIELDNAMES = (
    "model_id", "chunk_tokens", "duration_us", "dynamic_workspace_bytes", "output_bytes"
)

DECODE_EVENT_FIELDNAMES = (
    "model_id", "sequence_length", "block_size", "op_type", "duration_us", 
    "dynamic_workspace_bytes", "output_bytes"
)

PCIE_EVENT_FIELDNAMES = (
    "model_id", "block_size", "kv_block_bytes", "transfer_only_us", 
    "overlap_total_us", "dummy_compute_us", "exposed_transfer_us"
)
...
def generate_mock_prefill_events(
    ...
    for chunk in chunk_tokens:
        # Generate mock timing data (microseconds)
        base_us = 100 * (1 + chunk // 64)  # Scale with chunk size
        
        writer.write_row({
            "model_id": model_id,
            "chunk_tokens": chunk,
            "duration_us": base_us,
            "dynamic_workspace_bytes": 1024 * 1024 * (1 + chunk // 128),  # 1-2 MB
            "output_bytes": 512 * 1024 * chunk // 64,  # Scales with chunk
        })
        row_count += 1
...
def generate_mock_decode_events(
    ...
    op_types = ["gemv", "attention_fetch_compute", "reduction_overhead"]
        ...
        for block_size in chunk_sizes:
            for op_type in op_types:
                ...
                writer.write_row({
                    ...
                    "dynamic_workspace_bytes": 512 * 1024,  # Fixed workspace
...
def generate_mock_pcie_events(
    ...
    for block_size in chunk_sizes:
        ...
        exposed_transfer_us = transfer_only_us - 20 if transfer_only_us > 20 else 0  # Some overlap
