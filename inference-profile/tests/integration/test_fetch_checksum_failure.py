...
def _write_complete_remote_bundle(run_root: Path) -> dict[str, object]:
    ...
    relative_paths.extend(
        sorted(
            ...
            for path in bundle_paths.logs_dir.rglob("*")
            if path.is_file()
        )
    )
    ...
    for relative_path in relative_paths:
        if relative_path == checksum_relative_path:
            continue
        artifact_path = run_root / relative_path
        digest = compute_file_checksum(artifact_path)
        if relative_path == "raw/decode_events.csv":
            digest = "f" * 64
...
def test_fetch_stage_marks_manifest_fetch_failed_when_local_checksum_verification_fails(
    tmp_path: Path,
) -> None:
    run_id = f"fetch-checksum-{uuid4().hex}"
    ...
    try:
        ...
        (local_run_root / "environment.json").write_text(
            '{"revision": "known-good"}\n',
            encoding="utf-8",
        )
        ...
        assert local_manifest["final_status"] == "success"
        assert [
            entry["status"] for entry in local_manifest["final_status_history"]
        ] == ["success"]
        assert local_manifest["stages"] == remote_manifest["stages"]
        assert "status" not in local_manifest

        verification = verify_bundle(local_run_root)
        assert verification["status"] == "fetch_failed"
        assert "raw/decode_events.csv" in verification["checksum_mismatches"]
        assert "run_manifest.json" not in verification["checksum_mismatches"]
        assert (local_run_root / CHECKSUM_MANIFEST_RELATIVE_PATH).exists()
        assert (local_run_root / "raw" / "decode_events.csv").exists()
        assert (local_backup_root / "environment.json").read_text(encoding="utf-8") == (
            '{"revision": "known-good"}\n'
        )
