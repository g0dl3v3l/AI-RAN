...
def test_prefill_profile_smoke_writes_positive_durations_and_nonnegative_bytes(
    tmp_path: Path,
) -> None:
    if not torch.cuda.is_available():
        pytest.skip("CUDA is unavailable in this environment")

    output_root = tmp_path / "prefill-smoke"
    result = prefill_profile.profile_prefill_sweep(
    ...
    assert rows
    assert len(rows) == len(prefill_profile.PREFILL_OP_NAMES) * 2
