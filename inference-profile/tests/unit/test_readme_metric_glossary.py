...
def test_readme_defines_ttft():
    ...
    ttft_section = readme_text[readme_text.find("ttft_ms"):readme_text.find("ttft_ms") + 200]
...
def test_readme_defines_tpot():
    ...
    assert "per-output-token" in readme_text.lower() or "per-token" in readme_text.lower()
...
def test_readme_defines_kv_metrics():
    ...
    assert "kv_bytes_per_token" in readme_text
