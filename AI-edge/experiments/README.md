...
3. `smoke_validation.json` plus `smoke_response.jsonl` prove the in-flight request completed after restore or was replayed in a controlled way.
...
| In-flight request completed | `smoke_validation.json` and `smoke_response.jsonl` | `classification` is `smoke_completed_after_restore` or `smoke_replayed`, and the last response record has `status == "ok"` | The request finished after the restore path. `smoke_replayed` is weaker than `smoke_completed_after_restore`, but it still means the run produced a usable post-restore response. |

Use `smoke_validation.json` as the final decision record.

| `smoke_validation.json.classification` | Meaning | Verdict |
| --- | --- | --- |
| `smoke_completed_after_restore` | The strongest V0 result. The request completed after restore without needing replay. | pass |
| `smoke_replayed` | Restore completed, but the response had to be replayed after restore. | pass with caveat |
...
| `smoke_not_attempted` | The runtime never became preemptible, or you only ran the dry-run path. | expected skipped |
